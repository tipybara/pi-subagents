...
export class ConversationViewer implements Component {
  ...
  handleInput(data: string): void {
    ...
    if (matchesKey(data, "f")) {
      ...
      this.scrollOffset = Math.max(0, this.buildContentLines(this.lastInnerW).length - this.viewportHeight());
      this.tui.requestRender();
      return;
    }
  ...
  render(width: number): string[] {
    ...
    if (this.composer) {
      ...
      actions.push(th.fg("dim", `f ${this.focusMode ? "focus" : "full"}`));
      actions.push(th.fg("dim", `m ${MARKDOWN_MODE_LABELS[this.markdownMode()]}`));
      const navigation = "↑↓ scroll · PgUp/PgDn or Shift+↑↓ · Esc close";
      const footerRight = th.fg("dim", visibleWidth(actions.join(sep)) + navigation.length + 1 <= innerW
        ? navigation
        : "↑↓ · PgUp/PgDn/Shift+↑↓ · Esc close");
  ...
  private buildContentLines(width: number): string[] {
    ...
    const th = this.theme;
    const committedMessages = this.session.messages;
    // Pi adds the current assistant to the transcript only at message_end.
    const streamingMessage = this.focusMode ? this.session.agent?.state?.streamingMessage : undefined;
    const messages = streamingMessage?.role === "assistant" && !committedMessages.includes(streamingMessage)
      ? [...committedMessages, streamingMessage]
      : committedMessages;
    const lines: string[] = [];
    ...
    for (const msg of messages) {
      if (msg.role === "user") {
        ...
        const userLines = mode === "all"
          ? this.markdownLines(msg, text.trim(), width, false)
          : this.rawLines(text.trim(), width, false);
        if (isSteer) {
          lines.push(th.fg("warning", "✎ [Steer]"));
          lines.push(...userLines.map(line => paintSteerBg(th, line, width)));
        } else {
        ...
        for (const c of msg.content) {
          ...
          else if (c.type === "toolCall" && !this.focusMode) {
            toolCalls.push(c.name ?? (c as { toolName?: string }).toolName ?? "unknown");
          }
        }
        const text = textParts.join("\n").trim();
        if (this.focusMode && !text) continue;
        if (needsSeparator) lines.push(th.fg("dim", "───"));
      ...
      } else if (msg.role === "toolResult" && !this.focusMode) {
      ...
      } else if (msg.role === "bashExecution" && !this.focusMode) {
        const bash = msg;
        if (needsSeparator) lines.push(th.fg("dim", "───"));
    ...
    const assistantError = lastAssistant?.errorMessage?.trim()
      || (lastAssistant?.stopReason === "error" ? "Provider error (no details)."
        : lastAssistant?.stopReason === "aborted" ? "Assistant response aborted." : undefined);
    const errors = new Set([assistantError, this.record.error?.trim()]);
    for (const error of errors) {
      if (!error) continue;
      if (needsSeparator) lines.push(th.fg("dim", "───"));
      lines.push(th.fg("error", "[Error]"));
      lines.push(...this.rawLines(error, width, false).map(line => th.fg("error", line)));
      needsSeparator = true;
    }

    if (lines.length === 0) {
      const waiting = this.record.status === "running" || this.record.status === "queued";
      lines.push(th.fg("dim", waiting ? "(waiting for conversation text...)" : "(no conversation text to display)"));
    }

    // Streaming indicator for running agents
    if (this.record.status === "running" && (this.focusMode || this.activity)) {
      const act = !this.focusMode && this.activity
